...
void run(void) {
  while (running || !singleRequests.empty() || !allRequests.empty()) {
    do {
      ...
      if (!singleRequestsQueue.empty()){
        singleRequests.splice(singleRequests.begin(), singleRequestsQueue);
        TsanHappensAfter(&singleRequestsQueue);
      }
      if (!allRequestsQueue.empty()){
        allRequests.splice(allRequests.begin(), allRequestsQueue);
        TsanHappensAfter(&allRequestsQueue);
      }
...
int MPIX_Detach(MPI_Request *request, MPIX_Detach_callback *callback,
  ...
  if (flag) {
    ...
    std::unique_lock<std::mutex> lck(listMtx);
    auto* req = new singleRequest(request, callback, data);
    TsanHappensBefore(&singleRequestsQueue);
    singleRequestsQueue.push_back(req);
    listCv.notify_one();
  }
  return MPI_SUCCESS;
}
...
int MPIX_Detach_each(int count, MPI_Request array_of_requests[],
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (flag) {
      ...
      std::unique_lock<std::mutex> lck(listMtx);
      auto* req = new singleRequest(array_of_requests + i, callback, array_of_data[i]);
      TsanHappensBefore(&singleRequestsQueue);
      singleRequestsQueue.push_back(req);
      listCv.notify_one();
    }
...
int MPIX_Detach_all(int count, MPI_Request array_of_requests[],
  ...
  if (flag) {
    ...
    std::unique_lock<std::mutex> lck(listMtx);
    auto* reqs = new allRequest(count, array_of_requests, callback, data);
    TsanHappensBefore(&allRequestsQueue);
    allRequestsQueue.push_back(reqs);
    listCv.notify_one();
  }
  return MPI_SUCCESS;
}
